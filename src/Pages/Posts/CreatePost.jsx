...
const CreatePost = () => {
  ...
  const handleCreate = async () => {
    ...
    const postData = {
      ...
      TipoPost: [
        validTags.find(t => t.type === "tipo")?.label,
        validTags.find(t => t.type === "jogo")?.label,
        validTags.find(t => t.type === "personagem")?.label,
      ].filter(Boolean).join(" | "),

      PersonagemId:
        validTags.find((t) => t.type === "personagem")?.personagemId || null,
    ...
    if (
      !postData.PostTitulo ||
      !postData.PostConteudo ||
      !postData.TipoPost ||
      !postData.PersonagemId
    ) {
      alert(
        "Preencha todos os campos obrigatórios e selecione as tags corretamente!"
      );
      return;
    }
    createPost(postData)
    ...
    <div className="container-fluid col-12 col-md-5">
      <div className="d-flex flex-column gap-4">
        ...
        <div className="border-light border rounded-2">
          <div className="form-label text-center bg-light bg-opacity-50 fw-bolder m-0">
            Título
          </div>
          <input
            type=""
            className="input-group p-0 m-0 bg-opacity-10 bg-light border-0 py-1 px-3 text-light text-opacity-75"
            placeholder="Insira um título. Max caracteres: 100"
            value={titulo}
        ...
        <div className="border-light border rounded-2">
          <div className="form-conteudo form-label text-center bg-light bg-opacity-50 fw-bolder m-0 pt-1 pb-2">
            Conteúdo
          </div>
          <textarea
            type=""
            className="input-group p-0 m-0 bg-opacity-10 bg-light border-0 py-1 px-3 text-light text-opacity-75"
            rows="3"
            placeholder="Escreva algo sobre a postagem aqui..."
            value={conteudo}
            ...
            <div className="border-light border rounded-2">
              ...
              <div className="d-flex flex-wrap gap-2 p-2">
                {tags.map((tag, idx) => (
                  <div
                    key={idx}
                    className="bg-danger text-light px-2 py-1 rounded d-flex align-items-center gap-2"
                  >
                    <span>{tag.label}</span>
                    <button
        ...
        <button
          className="btn btn-outline-secondary text-light w-auto align-self-center fw-bolder"
          onClick={handleCancel}
        >
          Cancelar
        </button>
        <button
          className=" logNsign btn text-light w-auto align-self-center fw-bolder"
          onClick={handleCreate}
        >
          Criar discussão <i className="ms-2 bi bi-plus-circle-fill fs-6"></i>
        </button>
      </div>
